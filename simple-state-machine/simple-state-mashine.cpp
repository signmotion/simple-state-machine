...
namespace {
    ...
    struct my_machine_ : public msm::front::state_machine_def<my_machine_>
    {
        ...
        // transition actions
        void State2ToState3(msmf::none const&)       { std::cout << "my_machine::State2ToState3\n"; }
        void State3ToState4(msmf::none const&)       { std::cout << "my_machine::State3ToState4\n"; }
        // guard conditions
        bool always_true(msmf::none const& evt)
        {
            std::cout << "always_true" << std::endl;
            return true;
        }
        bool always_false(msmf::none const& evt)
        {
        ...
        struct transition_table : mpl::vector<
            ...
            //  +---------+-------------+---------+---------------------+----------------------+
           _row < State1  , msmf::none        , State2                                               >,
          a_row < State2  , msmf::none        , State3  , &p::State2ToState3                         >,
            //  +---------+-------------+---------+---------------------+----------------------+
            row < State3  , msmf::none        , State4  , &p::State3ToState4  , &p::always_true      >,
          g_row < State3  , msmf::none        , State4                        , &p::always_false     >,
    ...
    {        
        ...
        p.start(); 
        // this event will bring us back to the initial state and thus,
        // a new "loop" will be started
        p.process_event(event1());
        //p.process_event( msmf::none() );
